...
| `packages/paullette-web` | — | Not created. The place is reserved for the web interface of [issue #2](https://github.com/jeromeetienne/paullette/issues/2). |

The dependency runs one way only. `paullette-core` never imports from `paullette`, and it never will, because that one rule is what lets a second front end sit on the same agent. `paullette` reaches `paullette-core` by the package name, never by a relative path that climbs out of its own folder:
...
flowchart TD
	...
	WEB["paullette-web — not created<br/>issue number 2"]
...
The boundary is drawn, but one promise it implies is not yet kept. `paullette-core` still reads files through `node:fs` in `tools`, `history`, `memory`, `config_folder`, and `model_context_protocol`, so it does not run in a browser today.

## The folders inside `paullette-core`

| Folder | What it is responsible for |
| --- | --- |
| `src/agent/` | Points the software development kit at the endpoint, assembles the system prompt, and builds the agent object. |
...
4. **The `PermissionPrompt` is built**, and the **`ToolContext`** is built around it. Every tool from here on is handed that one object.
...
`PermissionAsker` is an interface, not a class, and that is the single most load-bearing decision in the tool layer. It is why `src/tools/` never imports from `src/terminal/`: the terminal package implements the interface with a prompt at the terminal, a unit test implements it with a fake that answers the same way every time, and a future web interface will implement it with something else entirely. The tool does not know and must not know.
...
sequenceDiagram
	...
	participant R as ReadlineInterface
...
`cli.ts` runs exactly one of five modes and then stops every Model Context Protocol server:
...
- **`paullette-core` is not free of Node.js**, so the promise that a web interface can sit on it is not yet kept.
...
**A new front end.** Implement `PermissionAsker`, build the `ToolContext` around it, and repeat the startup sequence above. Do not add anything to `paullette-core` that knows a terminal exists.
