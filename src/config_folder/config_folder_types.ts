...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
//	config_folder_types — the shapes read out of the .paullette folder
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////

/**
 * Accepts either a comma separated string or a list of strings, and always produces a list of strings. The
 * frontmatter of a `.paullette` file writes a list of tool names either way, because both spellings appear in the
...
/**
 * The Zod schema of the frontmatter of a file in `.paullette/agents`.
 */
export const agentFrontmatterSchema = z.object({
	/** The name of the subagent. paullette falls back to the file name when this field is absent. */
	...
	/** The model this subagent asks for. paullette ignores the value and uses the single configured model. */
	model: z.string().optional(),
});

/**
 * A subagent read from a file in `.paullette/agents`. paullette turns every subagent definition into a tool that the
 * main agent can call.
 */
...
/**
 * The Zod schema of the frontmatter of a file in `.paullette/commands`.
 */
export const commandFrontmatterSchema = z.object({
	...
	/** The names of the tools the command is allowed to call. paullette records the value but does not apply it. */
	'allowed-tools': toolNameListSchema.optional(),
	/** The model the command asks for. paullette ignores the value and uses the single configured model. */
	model: z.string().optional(),
});

/**
 * A slash command read from a file in `.paullette/commands`. The user types the name of the command, and paullette
...
/**
 * The Zod schema of the frontmatter of a `SKILL.md` file in `.paullette/skills`.
 */
export const skillFrontmatterSchema = z.object({
	/** The name of the skill. paullette falls back to the name of the folder when this field is absent. */
	name: z.string().optional(),
	/** The sentence that tells the agent when to load this skill. */
	description: z.string().optional(),
	/** The names of the tools the skill is allowed to call. paullette records the value but does not apply it. */
	'allowed-tools': toolNameListSchema.optional(),
});

/**
 * A skill read from a `SKILL.md` file in `.paullette/skills`. The agent sees the name and the description of every
...
/**
 * Where the single `.paullette` folder sits. paullette reads exactly one folder, at the project root, rather than
...
export type ConfigFolderPaths = {
	...
	/** The absolute path of the `.paullette` folder itself. */
	configFolderPath: string;
};

/**
 * Everything paullette read out of the `.paullette` folder.
 */
export type ConfigFolderContent = {
	/** Where the `.paullette` folder was found. */
...
export type InstructionDocument = {
	/** The absolute path of the `CLAUDE.md` file inside the `.paullette` folder. */
	filePath: string;
