import { type InstructionDocument, type SkillDefinition } from '../doublure_folder/doublure_folder_types.ts';
 ...
 * what a project with no `.doublure` content looks like.
 */
export type SystemPromptParts = {
	/** The folder the agent reads files from and runs shell commands in. */
	workingDirectoryPath: string;
	/** The instruction document read from `.doublure/CLAUDE.md`, or null when there is none. */
...
export class SystemPromptBuilder {
	...
	static build(parts: SystemPromptParts): string {
		...
		sections.push(
			[
				'You are doublure, a coding assistant working on the command line.',
				'',
