...
- `session_store.ts`: `SessionStore`, built with the path of the `.doublure/sessions` folder. It starts a session, saves one, and reads back the newest.
- `input_history_store.ts`: `InputHistoryStore`, built with the path of `.doublure/input_history.txt`.
...
- A session is written before the model is called, not only after it answers. That is what makes an interrupt safe: whenever doublure is stopped, the file on disk already holds everything said up to that moment.
...
- A session file is plain readable JSON on purpose. A person should be able to open one, see what was said, and delete it, without doublure being involved.
...
## Background
- Nothing is written when a session is started, only when a turn begins, so starting doublure and quitting without asking anything leaves no file behind.
