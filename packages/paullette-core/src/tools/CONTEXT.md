# Directory Context: `/packages/paullette-core/src/tools`
...
## Rules
- Nothing here imports from the `paullette` package, and nothing here imports from the `paullette-web` package. A tool asks for permission through the `PermissionAsker` interface declared in `tool_types.ts`, and `packages/paullette-cli/src/terminal/permission_prompt.ts` implements it. Turning that around would make the tools impossible to use from anywhere but a terminal.
