...
- `public/`: the page, the three stylesheet rules Bootstrap cannot reach, and the script sent to the browser.
...
- Everything the page looks like comes from Bootstrap classes. A rule is added to `public/paullette.css` only when no Bootstrap class can reach what it does, and it carries a comment saying which one was looked for and why it does not fit.
