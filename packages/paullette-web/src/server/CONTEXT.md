...
- `web_server.ts`: `WebServer`, built with the conversation and an address, with `listen()` and `close()`. It is the `node:http` module of Node.js and no web server library.
- `web_router.ts`: `WebRouter.route()`, which matches one method and one path to one answer.
...
- `web_static_files.ts`: `WebStaticFiles.read()`, which serves the three files of `public/`.
...
- The list of files `WebStaticFiles` serves is written out in the file. No path from a web address is ever resolved against a folder, so there is nothing to climb out of.
