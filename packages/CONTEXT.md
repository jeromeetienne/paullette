...
- `paullette-core` is not free of Node.js yet: `tools`, `history`, `memory`, and `config_folder` all read files through `node:fs`. The package boundary is drawn, the promise that `paullette-core` runs in a browser is not kept yet, and keeping it is its own piece of work. `paullette-web` does not need it kept: the whole of `paullette-web` runs in Node.js and only what sits under its `public/chat/` folder reaches a browser.
